'''
Everything is an MLP, blocks mostly pass through as is, other than a single mixing convolution (3x3) post-bottleneck.
'''
import traceback
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, images):
        ...
        latents = self.bottleneck(multiresolution_features)
        all_decoded = self.decoder(latents)
        return all_decoded, latents
...
class Bottleneck(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.down_proj_global = nn.Conv2d(config.hidden_size, config.bottleneck_dim,kernel_size=1,bias=False)
        
    def forward(self, x):
        x = self.down_proj_global(x)
        x = F.tanh(x)
        return x
...
class Decoder(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.receptive_field_expansion = nn.Conv2d(in_channels=config.bottleneck_dim, out_channels=config.hidden_size, kernel_size=3, padding=1, bias=False)
...
class PatchManager():
    def __init__(self, config, lr, device, name, model_type, label):
        ...
        if model_type == 'ae_mlp_hierarchical':
...
if __name__ == "__main__":
    ...
    celeb_manager = PatchManager(model_config, learning_rate, device, 'celeb', model_type='ae_mlp_hierarchical', label=prefix)
    
    # Hyperparameters etc.
    epochs = 10000
    samples_per_microbatch = 2#16
    base_grad_clip = 1.0
    ...
    if False:
        epoch_initial, iterations, total_microbatches, seen_ids, microbatches_per_step = celeb_manager.load('/mnt/f/models_trained/9e3e077d_celeb_epoch_4_step_1428_ae_mlp_hierarchical.pth')
        ...
        try:
            ...
            for microbatch_i in tqdm(range(len(data_loader))):
                ...
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                    predicted, _ = autoencoder_model(all_inputs)
