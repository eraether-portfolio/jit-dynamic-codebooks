...
### Noise and Self-Attention is All You Need
...
While countless models and scripts were trained, the final published code contains the training and inference code for 3 distinct models in the main pipeline.
- Autoencoder
- Context-Aware Dynamic Vocabulary Generator
- MaskGIT-like predictor making discrete predictions over dynamic vocabulary

Wrote and trained 2 more models in pytorch to explore different generation techniques
- VQVAE
- cGAN

Wrote inference code and tracked PSNR, FID, and other metrics

### The Research Effort
Ideated, trained, and investigated a novel training regime for a brand new generation technique.

Specifically, I believe that two contributions could feasibly become papers with more ablations and investigations:
- Multiple hypothesis generation using a single output head with no additional loss terms.
- Training a VQVAE without codebook collapse using dropout with no additional loss terms.

## Results
The results shown are still preliminary, but there is enough here to serve as a good proxy for what the final output will look like, and to get a sense of my capability as an AI researcher and engineer.  The cGAN and predictor will need perhaps one more week of GPU time to converge.
...
This is possible by first passing all tokens through a shared trunk, where they can mix (this project uses a CNN, but a ViT is fine).  Then, each token is duplicated 8 times, with different noise added to each duplicate.  These 8 tokens, through self-attention between each other, then push each other apart to cover the possibility space.  What you end up with is not 8 modes but 8 centroids that cover the possibility space.
